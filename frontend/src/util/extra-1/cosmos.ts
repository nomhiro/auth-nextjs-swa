...
export const getItemsByVector = async (embedding: number[]): Promise<any[]> => {
    return new Promise(async (resolve, reject) => {
        ...
        const { resources } = await container.items
            .query({
                query: "SELECT c.file_name, c.content, c.is_contain_image, c.image_blob_path, VectorDistance(c.content_vector, @embedding) AS SimilarityScore FROM c WHERE VectorDistance(c.content_vector, @embedding) > 0.88 ORDER BY VectorDistance(c.content_vector, @embedding)",
